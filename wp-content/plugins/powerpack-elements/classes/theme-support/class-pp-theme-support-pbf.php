 ...
 * @since 1.4.15
 */
final class PP_Header_Footer_PageBuilderFramework {
	/**
	...
	static public function setup_headers_and_footers()
	{
		if ( ! empty( PP_Header_Footer::$header ) ) {
			...
			add_action( 'wpbf_header', __CLASS__ . '::render_header' );
		}
		if ( ! empty( PP_Header_Footer::$footer ) ) {
			remove_action( 'wpbf_footer', 'wpbf_do_footer' );
			add_action( 'wpbf_footer', __CLASS__ . '::render_footer' );
		}
	}
	...
	{
		PP_Header_Footer::render_header( 'div' );
	}
...
PP_Header_Footer_PageBuilderFramework::init();